...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='f4e246bc93ffb68b33ed67c7896c379efa4207e7'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD -> master, refs/__gh__/pull/118387/rebase' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD -> master, refs/__gh__/pull/118387/rebase' =~ tag:\ (v[^ ,]+) ]]; then
